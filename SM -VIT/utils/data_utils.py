...
def get_loader(args):
    ...
    if args.dataset == 'dogs':
        ...
        if args.sm_vit:
            ...
            train_transform = transforms.Compose([
                transforms.Resize(
                    (args.resize_size, args.resize_size), Image.BILINEAR),
    ...
    #! Newly added
        ...
        print("HAM DATA SET Selected!!!!")
        trainset = HAM10000(args.dataset,
                            root=args.data_root,
                            is_train=True,
                            cropped=False,
                            transform=train_transform,
    ...
    if args.dataset == 'INat2017':
        ...
        train_transform = transforms.Compose([transforms.Resize((args.resize_size, args.resize_size), Image.BILINEAR),
            ...
            transforms.ColorJitter(
                                                  brightness=0.4, contrast=0.4, saturation=0.4),  # my add
            transforms.RandomHorizontalFlip(),
        ...
        test_transform = transforms.Compose([
            ...
            # transforms.Normalize([0.8416, 0.867, 0.8233], [0.2852, 0.246, 0.3262])])
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
        trainset = FGVC_aircraft(
            root=args.data_root, is_train=True, transform=train_transform)
        testset = FGVC_aircraft(root=args.data_root,
