...
class Generic_smvit_DS():
        ...
        if self.sm_vit:
            if self.full_ds:
                mask_u2n_list, x_u2n_list, y_u2n_list, h_u2n_list, w_u2n_list = \
                    mask_hw(full_ds=self.full_ds,
                            img_path=file_list_full, shape_hw=shape_hw_list)
            else:  # for debug
                ...
                mask_u2n, x_u2n, y_u2n, h_u2n, w_u2n = \
                    mask_hw(full_ds=self.full_ds, img_path=img_path,
                            shape_hw=(h_max_temp, w_max_temp))
                mask_temp, x, y, h, w = mask_u2n, x_u2n, y_u2n, h_u2n, w_u2n

        for ind, file in enumerate(file_list[:data_len]):

            if self.debug:
                print(f"{self.mode} file:", file)

            img_temp = scipy.misc.imread(
                os.path.join(self.root, self.base_folder, file))
            ...
            if self.gt_bbox:
                # x - distance from top up left (width), y - distance from top up left (height)
                x, y, w, h = bb_list[ind]

            if self.gt_parts:
                # list of 15 parts with [x, y] center corrdinates
                parts = parts_list[ind]
                ...
                for part_n in range(len(parts)):
                    ...
                    if part[0] != 0:
                        ...
                        mask_temp[int(y_min_p):int(y_max_p),
                                  int(x_min_p):int(x_max_p)] = 0
            ...
            if self.rand_crop:
                ...
                # 384 (92% - 0.96), 368 (84% - 0.92), 352 (77% - 0.88), 336 (70% - 0.84), 320 (64% - 0.80), 304 (57% - 0.75)
                h_crop_mid_img = int(h_max_img * 0.88)

                # w_crop_mid = 368 # 384 (92%), 368 (84%), 352 (77%), 336 (70%), 320 (64%), 304 (57%)
                # 384 (92% - 0.96), 368 (84% - 0.92), 352 (77% - 0.88), 336 (70% - 0.84), 320 (64% - 0.80), 304 (57% - 0.75)
                w_crop_mid_img = int(w_max_img * 0.88)

                # 40) #, 400-360) #, h - th)
                h_crop_min_img = random.randint(
                    0, (h_max_img - h_crop_mid_img))
                # 40)  #, 400-360) #, w - tw)
                w_crop_min_img = random.randint(
                    0, (w_max_img - w_crop_mid_img))
                ...
                if len(img_temp.shape) == 3:
                    img_temp = img_temp[int(h_crop_min_img):int(h_crop_max_img), int(
                        w_crop_min_img):int(w_crop_max_img), :]  # h, w, ch
                else:
                    img_temp = img_temp[int(h_crop_min_img):int(h_crop_max_img), int(
                        w_crop_min_img):int(w_crop_max_img)]  # h, w

                # Crop mask with bbox:
                mask_temp = mask_temp[int(h_crop_min_img):int(
                    h_crop_max_img), int(w_crop_min_img):int(w_crop_max_img)]
                ...
                if len(img_temp.shape) == 3:
                    if self.gt_parts:
                        for j in range(3):
                            img_temp[:, :, j] = img_temp[:, :, j] * \
                                mask_temp  # Black mask
                    else:
                        # test_img_temp = test_img_temp[int(y):int(y + h), int(x):int(x + w), :] # h, w, ch
                        img_temp = img_temp[int(y_min):int(y_max), int(
                            x_min):int(x_max), :]  # h, w, ch
                else:
                    if self.gt_parts:
                        # Black mask:
                        img_temp[:, :] = img_temp[:, :] * mask_temp
                    else:
                        img_temp = img_temp[int(y_min):int(
                            y_max), int(x_min):int(x_max)]  # h, w

                # Crop mask with bbox:
                if self.sm_vit or self.gt_bbox:
                    mask_temp = mask_temp[int(y_min):int(
                        y_max), int(x_min):int(x_max)]
            ##

            if ((img_temp.shape[0] != mask_temp.shape[0]) or (img_temp.shape[1] != mask_temp.shape[1])):
                print("[WARNING] Image shape does not match mask shape for sample:",
                      ind, ". \t", "Found shapes:", img_temp.shape, mask_temp.shape)
    ...
    def generic_preprocess_lowMem(self, file_list, file_list_full, shape_hw_list):
        ...
        print(
            f'[INFO] Pre-processing {self.mode} files in the low memory mode...')

        if self.sm_vit:
            if self.full_ds:
                print("####### New Log 2 #########")
                # print(self.full_ds)
                # print(file_list_full[0])
                # print("####### --- #########")
                mask_u2n_list, x_u2n_list, y_u2n_list, h_u2n_list, w_u2n_list = \
                    mask_hw(full_ds=self.full_ds,
                            img_path=file_list_full, shape_hw=shape_hw_list)
    ...
    def generic_getitem(self, index,  img, mask):
        ...
        if self.is_train:
            if self.rand_crop_im_mask:
                ...
                # Crop image:
                if len(img.shape) == 3:
                    img = img[int(h_crop_min_img):int(h_crop_max_img), int(
                        w_crop_min_img):int(w_crop_max_img), :]  # h, w, ch
                else:
                    img = img[int(h_crop_min_img):int(h_crop_max_img), int(
                        w_crop_min_img):int(w_crop_max_img)]  # h, w
                ...
                # Crop mask:
                mask = mask[int(h_crop_min_img):int(h_crop_max_img), int(
                    w_crop_min_img):int(w_crop_max_img)]
                ...
                if not self.flip_mask_as_image:  # normal
                    ...
                    if random.random() < 0.5:
                        ...
                        transform_img_flip = transforms.Compose([
                            ...
                            transforms.Resize(
                                (self.img_size, self.img_size), Image.BILINEAR),  # my for bbox

                            transforms.ColorJitter(
                                brightness=0.4, contrast=0.4, saturation=0.4),  # my add (FFVT)
                            # !!! FLIPPING in dataset.py !!!
        ...
        if self.crop_mask:
            ...
            # 384 (92% - 0.96), 368 (84% - 0.92), 352 (77% - 0.88), 336 (70% - 0.84), 320 (64% - 0.80), 304 (57% - 0.75)
            h_crop_mid = int(h_max_im * 0.84)
            # 384 (92% - 0.96), 368 (84% - 0.92), 352 (77% - 0.88), 336 (70% - 0.84), 320 (64% - 0.80), 304 (57% - 0.75)
            w_crop_mid = int(w_max_im * 0.84)
            ...
            # 40) #, 400-360) #, h - th)
            h_crop_min = random.randint(0, (h_max_im - h_crop_mid))
            # 40)  #, 400-360) #, w - tw)
            w_crop_min = random.randint(0, (w_max_im - w_crop_mid))

            h_crop_max = h_crop_mid + h_crop_min
            w_crop_max = w_crop_mid + w_crop_min

            cropped[int(h_crop_min):int(h_crop_max),
                    int(w_crop_min):int(w_crop_max)] = 0
            ...
            if not self.flip_mask_as_image:  # normal
                ...
                if flipped:
                    transform_mask = transforms.Compose([
                        ...
                        # Image.BILINEAR), # interpolation=T.InterpolationMode.NEAREST
                        transforms.Resize(
                            (mask_size, mask_size), interpolation=Image.NEAREST),
                        transforms.ToTensor()
                    ])
                else:
                    transform_mask = transforms.Compose([
    ...
    def generic_getitem_lowMem(self, index):
        ...
        img_temp = scipy.misc.imread(os.path.join(
            self.root, self.base_folder, file_temp))
        ...
        if self.rand_crop:
            ...
            # 40)  #, 400-360) #, w - tw)
            w_crop_min_img = random.randint(0, (w_max_img - w_crop_mid_img))
...
class CUB(Generic_smvit_DS):
    ...
    def __init__(self, ds_name, root, is_train=True, data_len=None, transform=None, sm_vit=True, low_memory=True, img_size=400):
        ...
        # if False - turn on RandomHorizontalFlip in data_utils !!!
        self.flip_mask_as_image = True
        ...
        if (self.sm_vit + self.gt_bbox + self.gt_parts) > 1:
            raise Exception(
                "Only one cropping mode (SM-ViT, bbox, parts) can be chosen")
        ...
        if self.is_train:
            self.file_list = [x for i, x in zip(
                train_test_list, img_name_list) if i]
            file_list_full = [os.path.join(self.root, self.base_folder, x) for i, x in zip(
                train_test_list, img_name_list) if i]
        else:
            self.file_list = [x for i, x in zip(
        ...
        if self.sm_vit:
            ...
            for img_name in self.file_list:
                img_temp = scipy.misc.imread(os.path.join(
                    self.root, self.base_folder, img_name))
                shape_hw_temp = [img_temp.shape[0],
                                 img_temp.shape[1]]  # h_max (y), w_max (x)
...
class dogs(Generic_smvit_DS):  # (Dataset):
    ...
    def load_split(self):
        if self.is_train:
            ...
            split = scipy.io.loadmat(
                join(self.root, 'splits/train_list.mat'))['annotation_list']
            labels = scipy.io.loadmat(
                join(self.root, 'splits/train_list.mat'))['labels']
        else:
            # split = scipy.io.loadmat(join(self.root, 'test_list.mat'))['annotation_list']
            # labels = scipy.io.loadmat(join(self.root, 'test_list.mat'))['labels']
            split = scipy.io.loadmat(
                join(self.root, 'splits/test_list.mat'))['annotation_list']
            labels = scipy.io.loadmat(
                join(self.root, 'splits/test_list.mat'))['labels']
...
class NABirds(Generic_smvit_DS):  # (Dataset):
    ...
    def __init__(self, ds_name, root, is_train=True, data_len=None, transform=None, sm_vit=True, low_memory=True, img_size=448):
        ...
        if self.is_train:
            self.label = [(self.label_map[x.target])
                          for x in self.data.iloc][:self.data_len]
        else:
            self.label = [(self.label_map[x.target])
                          for x in self.data.iloc][:self.data_len]
    ...
    def __getitem__(self, index):
        ...
        if self.sm_vit:
            if self.low_memory:
                ...
                img, target, imgname, mask = self.img[index], self.label[index], self.imgname[index], self.mask[index]
                img, mask = super(NABirds, self).generic_getitem(
                    index, img, mask)
...
class CarsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        car_class = torch.from_numpy(
            np.array(car_class.astype(np.float32))).long() - 1
...
class FGVC_aircraft():
    def __init__(self, root, is_train=True, data_len=None, transform=None):
        ...
        for line in train_label_file:
            train_img_label.append([os.path.join(
                train_img_path, line[:-1].split(' ')[0]), int(line[:-1].split(' ')[1])-1])
        for line in test_label_file:
            test_img_label.append([os.path.join(
                test_img_path, line[:-1].split(' ')[0]), int(line[:-1].split(' ')[1])-1])
        self.train_img_label = train_img_label[:data_len]
        self.test_img_label = test_img_label[:data_len]

    def __getitem__(self, index):
        if self.is_train:
            img, target = scipy.misc.imread(
...
class HAM10000(Generic_smvit_DS):  # (Dataset):
    folder = 'HAM10000'
    ...
    def __init__(self,
        ...
        self.images_folder = join(self.root, 'Images')
        self.annotations_folder = join(
            self.root, 'Annotation')  # TODO: Check and change
        # self._breeds = list_dir(self.images_folder)
        ...
        if self.sm_vit:
            ...
            print("####### New Log 1 #########")
            print(self.file_list[-1])  # output => ISIC_0029078.jpg
            # output => /home/uditha/snap/snapd-desktop-integration/current/Desktop/FYP/vit/sm-vit/ham10000/Images/ISIC_0029078.jpg
            print(file_list_full[-1])
            # output => ISIC_0029078.jpg
            print("self.file_list", len(self.file_list))
            # output => /home/uditha/snap/snapd-desktop-integration/current/Desktop/FYP/vit/sm-vit/ham10000/Images/ISIC_0029078.jpg
            print("file_list_full", len(file_list_full))
            print("####### --- #########")
            if self.low_memory:
                self.mask_u2n_list, self.x_u2n_list, self.y_u2n_list, self.h_u2n_list, self.w_u2n_list = \
                    super(HAM10000, self).generic_preprocess_lowMem(self.file_list,
                                                                    file_list_full,
                                                                    shape_hw_list
                                                                    )
                del shape_hw_list
                ...
                self.img, self.mask = \
                    super(HAM10000, self).generic_preprocess(self.file_list,
                                                             file_list_full,
                                                             shape_hw_list,
                                                             data_len
                                                             )
    ...
    def __getitem__(self, index):
        ...
        if self.sm_vit:
            if self.low_memory:
                target, imgname = self.label[index], self.imgname[index]
                img, mask = super(HAM10000, self).generic_getitem_lowMem(index)
            else:
                img, target, imgname, mask = self.img[index], self.label[index], self.imgname[index], self.mask[index]
                img, mask = super(HAM10000, self).generic_getitem(
                    index, img, mask)
    ...
    def load_split(self):
        if self.is_train:
            train_df = pd.read_csv(self.root+'/splits/train_list.csv')
            split = train_df['image_id'].tolist()
            labels = train_df['label'].tolist()
        else:
            test_df = pd.read_csv(self.root+'/splits/test_list.csv')
            split = test_df['image_id'].tolist()
