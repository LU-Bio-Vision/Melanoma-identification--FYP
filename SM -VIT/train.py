...
#! New method
# def drawConf(y_true, y_pred):
#     data_tmp = confusion_matrix(y_true, y_pred)
#     df_cm = pd.DataFrame(data_tmp, columns=np.unique(y_true), index = np.unique(y_true))
...
def simple_accuracy(preds, labels,mode=None):
    # drawConf(labels, preds)
    print("####",mode,"####")
    print("#### pred , label set ######")
    print(preds)
    print(labels)
    print("#### End --- pred , label set ######")
    return (preds == labels).mean()

def save_last_model(args, model):
    path = '/home/uditha/snap/snapd-desktop-integration/57/Desktop/FYP/vit/sm-vit/FinalModel'
    model_to_save = model.module if hasattr(model, 'module') else model
    mName = str(args.learning_rate)[2:] +"_"+ args.name
    model_checkpoint = os.path.join(path, "%s_checkpoint.bin" % mName)
    torch.save(model_to_save.state_dict(), model_checkpoint)
    logger.info("Saved Last model to [DIR: %s]", path)

def save_model(args, model):
    model_to_save = model.module if hasattr(model, 'module') else model
    mName = str(args.learning_rate)[2:] +"_"+ args.name
    model_checkpoint = os.path.join(args.output_dir, "%s_checkpoint.bin" % mName)
    torch.save(model_to_save.state_dict(), model_checkpoint)
...
def valid(args, model, writer, test_loader, global_step):
    ...
    accuracy = simple_accuracy(all_preds, all_label, "Validation of "+str(global_step))
...
def train(args, model):
    ...
    print("Try to save")
    #torch.save(model, '/home/uditha/snap/snapd-desktop-integration/current/Desktop/FYP/vit/sm-vit/FinalModel/model0.pt')
    torch.save(model, '/home/uditha/snap/snapd-desktop-integration/57/Desktop/FYP/vit/sm-vit/FinalModel/model_4.pt')
    save_last_model(args, model)
    print("Finished save")    
...
def main():
    ...
    parser.add_argument("--dataset", choices=["CUB", "dogs", "nabirds", "ham10000"], default="CUB",
                        help="Which downstream task.") #! New value added
